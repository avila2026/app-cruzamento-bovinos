...
async function seed() {
  ...
  // 1. Obter a fazenda padrão
  const { data: farms, error: farmError } = await supabase.from('farm').select('id').limit(1);
  if (farmError || !farms || farms.length === 0) {
    console.error("Erro ao obter fazenda. Certifique-se de ter executado o init_db.sql no editor SQL do Supabase.", farmError);
    return;
  }
  const farmId = farms[0].id;
  console.log("Fazenda encontrada ID:", farmId);
