...
upload de fotos, e um assistente de IA (chat) para interpretar DEPs / sugerir acasalamentos.

- **Pasta do app:** `f:\Download\touros fazenda\app-cruzamento`
- **Stack:** React 19 · Vite 8 · TypeScript 6 · Tailwind 4 · react-router-dom 7 · Supabase
...
- **Rodar:** `npm run dev` → http://localhost:5173 (proxy `/ollama` ativo). Build: `npm run build` (passa limpo).
...
- Tabelas-chave: `farm`, `animal`, `animal_relation` (PAI/MAE/AVO_MATERNO, texto via `related_name`),
  ...
  (mapeia conceito → `code_geneplus`/`code_pmgz`/`code_ancp`), `file_asset` (tipo FOTO).
- Dados: 1 touro real "Carbono FIV da S.Nice" já carregado com DEPs dos 3 programas.
...
- **Config:** `vite.config.ts` (proxy `/ollama` → `http://localhost:11434`), `vercel.json`
  (rewrites SPA), `.env.local` com vars Supabase + Ollama.
...
Confirmar com o usuário qual frente atacar: (a) destravar Vercel pelo painel, ou
(b) criar o bucket de fotos no Supabase, ou (c) evoluir features do app. O deploy e o bucket
dependem de ações na conta do usuário — guie, não tente forçar automaticamente.
